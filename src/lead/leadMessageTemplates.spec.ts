...
describe('leadMessageTemplates', () => {
  describe('renderLeadMessage — fixed (no-payload) templates', () => {
    ...
    it('every Offer Status template (new in this revision) renders its exact spec copy', () => {
      expect(renderLeadMessage('OFFER_COLLECTED').text).toBe(
        '✅ I have collected the offer.',
      );
      expect(renderLeadMessage('OFFER_REDEEMED').text).toBe(
        '✅ Offer redeemed successfully.',
      );
      expect(renderLeadMessage('VISIT_CANCELLED').text).toBe(
        '❌ Unable to visit today.',
      );
      expect(renderLeadMessage('RESCHEDULE_REQUEST').text).toBe(
        '🔄 Can we reschedule?',
      );
    });

    it('ignores any payload passed for a fixed template — it cannot be used to smuggle text', () => {
      const result = renderLeadMessage('CONFIRM_THANKS', {
        text: 'this is not the real message',
      });
  ...
  describe('renderLeadMessage — SCHEDULE template', () => {
    it('renders the exact "Schedule Meeting" phrasing from a {date, time} payload', () => {
      expect(
        renderLeadMessage('SCHEDULE_AVAILABLE_ON', {
          date: '2026-07-29',
          time: '17:30',
        }).text,
    ...
    it('echoes the payload back for storage', () => {
      ...
      expect(
        renderLeadMessage('SCHEDULE_AVAILABLE_ON', payload).payload,
      ).toEqual(payload);
    });
    ...
    it.each([
      ...
      [
        'malformed time (12h with AM/PM)',
        { date: '2026-07-29', time: '5:30 PM' },
      ],
      ['out-of-range time', { date: '2026-07-29', time: '25:99' }],
      ['non-calendar date', { date: '2026-02-30', time: '17:30' }],
    ])('rejects %s', (_label, payload) => {
      expect(() => renderLeadMessage('SCHEDULE_AVAILABLE_ON', payload)).toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('renderLeadMessage — LOCATION_LIVE', () => {
    it('renders the fixed text and stores lat/lng in payload', () => {
      const result = renderLeadMessage('LOCATION_LIVE', {
        lat: 12.9716,
        lng: 77.5946,
      });
    ...
    it.each([
      ...
      expect(() => renderLeadMessage('LOCATION_LIVE', payload)).toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('renderLeadMessage — whitelist enforcement', () => {
    it('rejects a key that is not in the catalog at all — this is what blocks free text', () => {
      expect(() => renderLeadMessage('Hey, are you around today?')).toThrow(
        BadRequestException,
      );
      expect(() => renderLeadMessage('')).toThrow(BadRequestException);
      expect(() => renderLeadMessage('meeting_when')).toThrow(
        BadRequestException,
      ); // case-sensitive, not a fuzzy match
    });
  });
  ...
  describe('getPublicTemplateCatalog', () => {
    ...
    it('Quick Actions has no payload-requiring items — every item is immediately sendable, no tab switch needed', () => {
      ...
      expect(quickActions.items.every((i) => i.requiresPayload === false)).toBe(
        true,
      );
    });
    ...
    it('flags exactly the SCHEDULE and LOCATION_LIVE templates as requiring a payload', () => {
      ...
      const needsPayload = flat
        .filter((i) => i.requiresPayload)
        .map((i) => i.key)
        .sort();
      expect(needsPayload).toEqual(
        ['LOCATION_LIVE', 'SCHEDULE_AVAILABLE_ON'].sort(),
      );
    });
  });
  ...
  describe('getAnalyticsEvents', () => {
    ...
    it('a schedule send maps to both DATE_SHARED and TIME_SHARED', () => {
      expect(getAnalyticsEvents('SCHEDULE_AVAILABLE_ON')).toEqual([
        'DATE_SHARED',
        'TIME_SHARED',
      ]);
    });
    ...
    it('matches the requested example event names for the requested example templates', () => {
      ...
      expect(getAnalyticsEvents('MEETING_ARRIVED')).toEqual([
        'MEETING_CONFIRMED',
      ]);
      expect(getAnalyticsEvents('OFFER_COLLECTED')).toEqual([
        'OFFER_COLLECTED',
      ]);
