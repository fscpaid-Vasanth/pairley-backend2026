...
export type LeadMessageCategory =
  | 'QUICK_ACTIONS'
  | 'SCHEDULE'
  | 'LOCATION'
  | 'OFFER_STATUS';
...
export const LEAD_MESSAGE_TEMPLATES: Record<string, LeadMessageTemplateDef> = {
  ...
  MEETING_COUNTER: {
    ...
    messageType: 'STATEMENT',
    requiresPayload: false,
    label: '🏪 Shall we meet at the shop counter?',
    text: '🏪 Shall we meet at the shop counter?',
    analyticsEvents: ['MEETING_REQUESTED'],
  },
  MEETING_ARRIVED: {
    category: 'QUICK_ACTIONS',
    messageType: 'STATEMENT',
    requiresPayload: false,
    label: '🚗 I have reached the shop.',
    text: '🚗 I have reached the shop.',
  ...
  CONFIRM_LOOKING_FORWARD: {
    ...
    messageType: 'STATEMENT',
    requiresPayload: false,
    label: '😊 Looking forward to meeting.',
  ...
  SCHEDULE_AVAILABLE_ON: {
    ...
    render: (p) =>
      `📅 I will be available on ${formatDate(p.date)} at ${formatTime(p.time)}`,
  ...
  LOCATION_SHOP_ENTRANCE: {
    ...
    requiresPayload: false,
    label: '📍 Shop Entrance',
    text: '📍 Meeting Location: Shop Entrance',
    analyticsEvents: ['LOCATION_SHARED'],
  },
  LOCATION_PARKING_AREA: {
    ...
    requiresPayload: false,
    label: '📍 Parking Area',
    text: '📍 Meeting Location: Parking Area',
    analyticsEvents: ['LOCATION_SHARED'],
  },
  LOCATION_LIVE: {
    category: 'LOCATION',
  ...
  OFFER_COLLECTED: {
    ...
    messageType: 'STATEMENT',
    requiresPayload: false,
    label: '✅ I have collected the offer.',
    text: '✅ I have collected the offer.',
  ...
  OFFER_REDEEMED: {
    ...
    messageType: 'STATEMENT',
    requiresPayload: false,
    label: '✅ Offer redeemed successfully.',
  ...
  VISIT_CANCELLED: {
    ...
    messageType: 'STATEMENT',
    requiresPayload: false,
    label: '❌ Unable to visit today.',
    text: '❌ Unable to visit today.',
...
const CATEGORY_ORDER: LeadMessageCategory[] = [
  'QUICK_ACTIONS',
  'SCHEDULE',
  'LOCATION',
  'OFFER_STATUS',
...
export function getPublicTemplateCatalog() {
  ...
  for (const [key, def] of Object.entries(LEAD_MESSAGE_TEMPLATES)) {
    const item = {
      ...
      requiresPayload: def.requiresPayload,
    };
